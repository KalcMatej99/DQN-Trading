...
class DataAutoPatternExtractionAgent(Data):
    def __init__(self, data, state_mode, action_name, device, gamma, n_step=4, batch_size=50, window_size=1,
        ...
        if state_mode == 1:  # OHLC
            ...
            # window_size * OHLC
            self.state_size = window_size * (4 + 8)
            temp_states = []
            for i, row in self.data.loc[:, ['volume_norm', 'open_norm', 'high_norm', 'low_norm', 'close_norm', "adx_norm", "rsi_norm", "fastd_norm", "fastk_norm", "tema9_norm", "tema21_norm", "tema100_norm"]].iterrows():
                if i < window_size - 1:
